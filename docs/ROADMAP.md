...
Current version: **1.18.2** (see `../CHANGELOG.md` for full detail on every
...
3. **Phase 44 — Internet P2P Connectivity** (core primitives complete: 44.1 `locator.py`+storage landed as `1.18.0`, 44.2 `endpoint_update.py` landed as `1.18.1`, 44.3 `link.py` Add-by-Link landed as `1.18.2` — all three are crypto/data/storage only; `/link` UI commands, `endpoint_update` wire-protocol integration, and actually initiating a connection to a decoded link's endpoint are still unwired) ← next
